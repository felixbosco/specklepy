...
class TestAlignment(unittest.TestCase):
    ...
    def test_get_pad_vectors(self):
        alignment.derive_pad_vectors(self.shifts, cube_mode=(len(self.cube_shape) == 3))

    def test_pad_array(self):
        pad_vectors, ref_pad_vector = alignment.derive_pad_vectors(self.shifts, cube_mode=False)
        padded = alignment.pad_array(np.ones(self.image_shape), pad_vectors[1], mode='same', reference_image_pad_vector=ref_pad_vector)
        imshow(padded)
        pad_vectors, ref_pad_vector = alignment.derive_pad_vectors(self.shifts, cube_mode=True)

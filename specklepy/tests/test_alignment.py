...
class TestAlignment(unittest.TestCase):
    ...
    def test_get_shift(self):
        ...
        shift_peak = alignment.estimate_shift(image=image, reference_image=ref_image, mode='peak')
        shift_max  = alignment.estimate_shift(image=image, reference_image=ref_image, mode='maximum')
        shift_corr = alignment.estimate_shift(image=image, reference_image=ref_image, mode='correlation')
    ...
    def test_get_shifts(self):
        ...
        alignment.estimate_shifts(self.files, mode='correlation')

    def test_get_pad_vectors(self):
        alignment.derive_pad_vectors(self.shifts, cube_mode=(len(self.cube_shape) == 3), return_reference_image_pad_vector=True)

    def test_pad_array(self):
        pad_vectors, ref_pad_vector = alignment.derive_pad_vectors(self.shifts, cube_mode=False, return_reference_image_pad_vector=True)
        padded = alignment.pad_array(np.ones(self.image_shape), pad_vectors[1], mode='same', reference_image_pad_vector=ref_pad_vector)
        imshow(padded)
        pad_vectors, ref_pad_vector = alignment.derive_pad_vectors(self.shifts, cube_mode=True, return_reference_image_pad_vector=True)

...
def holography(params, debug=False):
    ...
    source_extractor_kwargs = None  # {'fwhm': source_extraction.get('starfinderFwhm', 10)}
    ...
    out_file_name = paths.get('outFile')
    ...
    reconstruction = Reconstruction(in_files=in_files, mode=mode, integration_method='ssa',
                                    ...
                                    in_dir=in_dir, tmp_dir=tmp_dir, out_file=out_file_name,
    ...
    while True:
        ...
        f_object.coadd_fft(mask_hot_pixels=mask_hot_pixels, bootstrap_number=bootstrap_number)
        ...
        logger.info(f"Updating the image data in {out_file_name!r} with the recent reconstruction")
